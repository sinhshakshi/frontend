...
import React, { useEffect, useState } from 'react';
import { MaterialReactTable } from 'material-react-table';
import { Box, Typography, Chip, Grid, CardContent, Card, Button } from '@mui/material';
  ...
  const getRowColor = (plan) => {
    switch (plan) {
      ...
      case '90 Days':
        return '#81c784'; // Dark Green
      case '3 Days':
        return '#d27d2f'; // Light Brown (Updated from Dark Brown)
      case '25 Days':
        return '#9575cd'; // Purple
      default:
        return '#e0e0e0'; // Light Gray
...
// Download `No Plan` students' email IDs as Excel
const downloadNoPlanExcel = () => {
  // Adjusted filter condition to catch empty, null, or undefined 'selectedPlan'
  const noPlanData = data.filter((item) => !item.selectedPlan || item.selectedPlan.trim().toLowerCase() === '');

  console.log("Filtered No Plan Data:", noPlanData);  // Log the filtered data for debugging

  const exportData = noPlanData.map((item) => ({
    email_id: item.email_id,  // Only export the email_id field
  }));

  console.log("Export Data:", exportData);  // Log the final data for export

  const ws = XLSX.utils.json_to_sheet(exportData); // Convert the data to a sheet
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'No Plan Students');
  const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
  const excelFile = new Blob([excelBuffer], { type: 'application/octet-stream' });
  saveAs(excelFile, 'No_Plan_Students_Emails.xlsx');
};
  ...
  return (
    <Box sx={{ padding: 2 }}>
      ...
      {/* Displaying plan totals in cards */}
      <Grid container spacing={3}>
        {planData.map(({ plan, count }) => (
          <Grid item xs={12} sm={6} md={4} key={plan}>
            <Card
      ...
      {/* Button to download No Plan students as Excel */}
      <Button
        variant="contained"
        color="primary"
        onClick={downloadNoPlanExcel}
        sx={{ marginBottom: 3, marginTop: 2 }}
      >
        Download No Plan Students as Excel
      </Button>
